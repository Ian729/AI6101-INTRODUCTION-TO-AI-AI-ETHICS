...
if __name__ == '__main__':
    ...
    #         print(action)
        ...
        rewards = []
        # if i == 500:
        #     f = open("Q_table_middle.csv","w")
        #     for key in sorted(agent.Q.keys()):
        #         print(f"{key}:{agent.Q[key][0]:.2f};{agent.Q[key][1]:.2f};{agent.Q[key][2]:.2f};{agent.Q[key][3]:.2f};"+["UP","DOWN","LEFT","RIGHT"][np.argmax(agent.Q[key])])
        #         f.write(f"{key}:{agent.Q[key][0]:.2f};{agent.Q[key][1]:.2f};{agent.Q[key][2]:.2f};{agent.Q[key][3]:.2f};"+["UP","DOWN","LEFT","RIGHT"][np.argmax(agent.Q[key])]+"\n")
        #     f.close()            
